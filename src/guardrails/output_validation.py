...
def validate_system_recommendation(
    ...
    if arbitrator_output.confidence_score < 0.60:
        flags.append(
            f"Confidence score {arbitrator_output.confidence_score:.2f} "
            "is below the 60% safety threshold. Review required."
        )

    # 2. Check for arbitrator conflicts
    if arbitrator_output.agent_agreement == "conflict":
        flags.append(
            "Agent disagreement detected. Arbitrator output flagged for conflict resolution."
        )
    ...
    requires_review_override = (
        arbitrator_output.confidence_score < 0.60 or arbitrator_output.agent_agreement == "conflict"
    )
