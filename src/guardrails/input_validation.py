...
def validate_application_input(application_data: Dict[str, Any], documents: List[Dict[str, Any]]) -> Tuple[bool, List[str]]:
    """
    ...
    required_fields = ["name", "income", "monthly_debt", "loan_amount", "property_value", "employment_months"]
    ...
    if len(documents) < 3:
        errors.append(f"Application has only {len(documents)} documents. Minimum of 3 required documents is mandated by KYC policy.")
    ...
    for i, doc in enumerate(documents):
        ...
        if filename:
            ...
            if ext not in allowed_types:
                errors.append(f"Invalid file type for {filename}. Only PDF, JPG, PNG, and TXT are accepted.")
