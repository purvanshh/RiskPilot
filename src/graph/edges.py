...
def route_after_kyc(state: LoanApplicationState) -> Literal["credit", "human_review", "retry"]:
    """
...
def route_after_arbitrator(state: LoanApplicationState) -> Literal["human_review"]:
    ...
    - All recommendations (approve, deny, review) must go through mandatory Human-in-the-Loop review.
    """
