...
if not applications:
    ...
    app_options = {
        f"{app['application_id']} - {app['applicant_data']['name']}": app for app in applications
    }
...
if selected_app:
    ...
    with col2:
        ...
        pv = selected_app["applicant_data"]["property_value"]
        st.write(f"**Collateral / Property Value:** ${pv:,}")
        ltv = (
            selected_app["applicant_data"]["loan_amount"]
            / selected_app["applicant_data"]["property_value"]
        )
    ...
    with col3:
        ...
        st.write(
            f"**Employment History:** {selected_app['applicant_data']['employment_months']} months"
        )
        exp = selected_app.get("expected_recommendation", "N/A").upper()
        st.write(f"**Expected Outcome:** :orange[{exp}]")
        st.write(f"**Scenario:** *{selected_app.get('description', '')}*")
        st.markdown("</div>", unsafe_allow_html=True)

    # Initialize session state for graph execution
    if (
        "graph_state" not in st.session_state
        or st.session_state.get("current_app_id") != selected_app["application_id"]
    ):
    ...
    if st.button("⚡ Run Multi-Agent Underwriting pipeline"):
        with st.spinner("Orchestrating agents via LangGraph..."):
            ...
            # Graph: kyc -> credit -> policy -> arbitrator -> human_review
    ...
    if state_data:
        ...
        if arb_out:
            ...
            rec_upper = arb_out.recommendation.upper()
            conf = arb_out.confidence_score
            agree = arb_out.agent_agreement.upper()
            st.markdown(
                f"""
            <div style="padding:1rem;border-radius:8px;border:2px solid #3b82f6;">
                <h3>Arbitrator: <b style="color:{rec_color}">{rec_upper}</b></h3>
                <p><b>Confidence Score:</b> {conf:.2f} | <b>Agreement:</b> {agree}</p>
        ...
        with col_kyc:
            ...
            if kyc_out:
                status = "Passed" if not kyc_out.get("missing_critical_docs") else "Failed"
                st.write(f"**Verification status:** {status}")
                flag = "⚠️ Yes" if kyc_out.get("fraud_flag") else "No"
                st.write(f"**Fraud/Suspicion Flag:** {flag}")
        ...
        if not st.session_state["officer_action_submitted"]:
            with st.form("hitl_review_form"):
                ...
                if submit:
                    if (
                        "override" in decision or default_rec != decision
                    ) and not override_reason.strip():
                        st.error(
                            "Override reasoning is mandatory when overriding agent recommendation."
                        )
