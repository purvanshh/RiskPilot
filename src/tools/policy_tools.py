...
def policy_retriever(query: str) -> List[str]:
    ...
    policy_chunks = [
        # fmt: off
        "Credit Score Policy: The absolute minimum credit score required for auto-approval is 650. "
        "Any score below 650 must be marked as review_required or deny.",
        "Credit Score Policy Tiers: Tier 1 (Low Risk) >= 720, Tier 2 (Medium Risk) 650-719, "
        "Tier 3 (High Risk) 600-649, Tier 4 (Unacceptable Risk) < 600.",
        "Debt-to-Income (DTI) Policy: The maximum DTI for standard approvals is 45% (0.45). "
        "Under no circumstances shall an applicant be approved with a DTI exceeding 50% (0.50).",
        "DTI Borderline Policy: DTI ratios between 40% and 45% are considered borderline "
        "and should prompt the Arbitrator agent to flag for manual review.",
        "Loan-to-Value (LTV) Policy: The maximum allowable Loan-to-Value (LTV) ratio "
        "for standard mortgage/property-backed loans is 80% (0.80).",
        "LTV Exception Policy: LTV ratios up to 85% (0.85) may be approved only if "
        "the applicant's credit score is greater than 720 and their DTI is under 35%. "
        "Any LTV exceeding 85% is a hard policy violation.",
        "Employment Stability Policy: Applicants must have a minimum of 12 months "
        "of continuous employment history with their current employer.",
        "Employment Policy Exceptions: Less than 12 months (e.g., 6-11 months) may be accepted "
        "if the applicant was in a similar role previously with no gaps, subject to manual review. "
        "Less than 6 months is direct denial.",
        "Income Verification Policy: To verify income, the applicant must provide at least two "
        "documents: bank statement showing payroll deposits, pay slip, or employment letter.",
    ]
...
def policy_validator(
    ...
    if ltv > 0.85:
        ...
        if credit_score <= 720 or dti >= 0.35:
            passed = False
            violations.append(
                f"LTV ratio {ltv:.2%} exceeds the standard 80% "
                f"(exception conditions not met: score={credit_score}, DTI={dti:.2%})."
            )
    ...
    if employment_months < 12:
        ...
        passed = False
        violations.append(
            f"Employment tenure of {employment_months} months "
            "is below the standard requirement of 12 months."
        )
