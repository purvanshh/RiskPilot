...
from src.graph.state import LoanApplicationState, CreditRiskOutput
...
def credit_node(state: LoanApplicationState) -> Dict[str, Any]:
    ...
    try:
        ...
        reasoning = (
            f"Calculated Credit Score: {credit_score} based on annual income of ${income:.2f}, "
            f"monthly debt of ${monthly_debt:.2f}, and employment tenure of {employment_months} months. "
            f"DTI ratio is {dti:.2%}, leading to a risk classification of {risk_category.upper()}."
        )
