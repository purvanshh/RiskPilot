...
from src.tools.document_tools import parse_document, extract_fields, validate_fields
...
def kyc_node(state: LoanApplicationState) -> Dict[str, Any]:
    ...
    if not state.documents:
        ...
        try:
            ...
            mock_docs = [
                {"type": "id_proof", "text": "ID Proof: Alice Johnson, DOB: 12/10/1990"},
    ...
    for doc in extracted_docs:
        if doc.document_type == "pay_slip":
            pay_slip_income = doc.extracted_fields.get("income_monthly") or doc.extracted_fields.get("income", 0) / 12
        elif doc.document_type == "bank_statement":
            bank_statement_income = doc.extracted_fields.get("income_monthly") or doc.extracted_fields.get("income", 0) / 12
    ...
    if pay_slip_income and bank_statement_income:
        # If mismatch is > 10%
        if abs(pay_slip_income - bank_statement_income) / max(pay_slip_income, bank_statement_income) > 0.10:
    ...
    kyc_output = {
        ...
        "verified_fields": {
            "name": next((d.extracted_fields.get("name") for d in extracted_docs if d.extracted_fields.get("name")), None),
            "income": next((d.extracted_fields.get("income_monthly") * 12 for d in extracted_docs if d.extracted_fields.get("income_monthly")), state.applicant_data.get("income")),
            "employer": next((d.extracted_fields.get("employer") for d in extracted_docs if d.extracted_fields.get("employer")), None),
        }
    }
